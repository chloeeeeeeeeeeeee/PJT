...
public class QnaController {
	...
	@ApiOperation(value = "QnA 질문 하기", notes = "글 제목, 내용, 비밀글여부", response = List.class)
	@PostMapping("/create")
...
//		User user = userDao.findByUserSeq(1);
	...
	@ApiOperation(value = "마이페이지에서 QnA 게시판 불러오기", notes = "페이지 번호를 기준으로 글 목록 불러옴", response = List.class)
...
//		User user = userDao.findByUserSeq(1);
	...
	@ApiOperation(value = "QnA 질문 및 답변 상세내용", notes = "qnaSeq, qnaSecret로 글 상세내용을 불러온다. qnaSecret=1인데 받아온 userSeq와 qnaSeq의 userSeq가 다를 경우 null return", response = List.class)
	@PostMapping("/read")
...
//			User user = userDao.findByUserSeq(1);
	...
	@ApiOperation(value = "QnA 질문 수정", notes = "qnaSeq, 글제목, 내용, 비밀글 여부를 받아 질문 수정", response = List.class)
	@PostMapping("/update")
